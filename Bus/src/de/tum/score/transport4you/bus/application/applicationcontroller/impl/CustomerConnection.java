...
public class CustomerConnection {
	...
	public synchronized void incomingData(Object data) {
		...
		if (data instanceof BlobEnvelope) {
			...
			try {
				if (envelope
						.checkConsistency(DataControllerInterfaceCoordinator
								.getSettingsDataController()
								.getKeyConfiguration().getBlobPublicKey())) {
					// Consistent Envelope
					this.customerBlob = envelope.getPublicBlobEntry();
					// Check if there is an ETicket in the Blob
					logger.debug("Checking for tickets");
					ArrayList<ETicket> eTicketList = envelope
							.getPublicBlobEntry().geteTicketList();
					if (eTicketList.isEmpty()) {
						...
						if (validTicket == null) {
							...
							if (!validTicket.isInvalidated()) {
								...
								newBlob = new BlobEnvelope(
										envelope.getPublicBlobEntry(),
										DataControllerInterfaceCoordinator
												.getSettingsDataController()
												.getKeyConfiguration()
												.getBlobPrivateKey());
							...
							message.setBlob(newBlob); // may be null, but that
														// is ok
							ConnectionManagerInterfaceCoordinator
									.getConnection().sendData(
				...
				logger.error("Error while loading the crypto libraries: "
						+ e.getMessage());
				this.disconnect();
				return;
			} catch (IOException e) {
				...
				logger.error("Error while loading the local configuration: "
						+ e.getMessage());
				this.disconnect();
				return;
				...
				logger.error("Error while doing the check in: "
						+ e.getMessage());
				this.disconnect();
				return;
			}
			...
			// Retrieve the ETicketType => note that Customer may fake,
			// therefore compare the types itself
			List<ETicketType> eTicketTypeList = DataControllerInterfaceCoordinator
					.getETicketTypeDataController().getAllETicketTypes();
			ETicketType selectedType = null;
			for (ETicketType eticketType : eTicketTypeList) {
				if (eticketType.getName().equals(
						purchaseMessage.getSelectedETicketType().getName())) {
			...
			try {
				if (this.customerBlob == null) {
					...
					ConnectionManagerInterfaceCoordinator.getConnection()
							.sendData(this.getContext(), null);
				} else {

					try {
						// Check Account Type and perform payment process
						if (this.customerBlob.getAccountType().equals(
								DataControllerInterfaceCoordinator
										.getSettingsDataController()
										.getApplicationConfiguration()
										.getPrepayAccountRepresentation())) {
							// Prepay account => check balance

							if (this.customerBlob.getAccountBalance() >= selectedType
									.getPrice()) {
								// Ticket can be sold
								logger.debug("Enough money on account to buy the ticket(s)");
								this.createETicketsAndSendNewBlob(selectedType,
										true);

							} else {
								// Ticket can not be sold
								logger.debug("Ticket can not be sold due to not enough money on account");
								ETicketPurchaseFailedMessage failedMessage = new ETicketPurchaseFailedMessage(
								...
								// disconnect() //wait some time because sending
			...
			if (transaction == null) {
				...
				try {
					DataControllerInterfaceCoordinator
							.getPersistentDataController().save(apoList);
				} catch (PersistenceException e) {
					logger.error("Could not save the transaction: "
							+ e.getMessage());
					disconnect();
					return;
				}

				// Disconnect here: Normally the mobile system would have to
				// send keepalive to notify the
				// bus about its presence, this is not implemented in the frist
				// increment
	...
	private void createETicketsAndSendNewBlob(ETicketType selectedType,
			boolean prepay) throws InvalidKeyException,
			ConfigurationLoadingException, SendDataException {

		logger.debug("Create ETickets for ETicketType: "
		...
		/* for (int i = 0; i < selectedType.getAmountTickets(); i++) { ETicket
		 * newTicket = new ETicket(); newTicket.setInvalidated(false);
		...
		for (ETicket localETicket : loggingETicketList) {
			...
			if (localETicket.getInvalidatedAt() != null) {
				// That is the local representation of the ETicket currently
				// used for boarding
				try {
		...
		if (prepay) {
			...
			this.customerBlob.setAccountBalance(this.customerBlob
					.getAccountBalance() - selectedType.getPrice());
			prePayTrans.setAmount(this.customerBlob.getAccountBalance());
		...
		// Calculate and send new Blob
		BlobEnvelope newBlob = new BlobEnvelope(this.customerBlob,
				DataControllerInterfaceCoordinator.getSettingsDataController()
						.getKeyConfiguration().getBlobPrivateKey());
		...
		try {
			DataControllerInterfaceCoordinator.getPersistentDataController()
					.save(loggingList);
		} catch (PersistenceException e) {
			logger.error("Error while accessing persitence layer, cannot save logging information");
			// TODO: here exception messages need to be passed to the gui
			// controller to inform user about problem with the system
			disconnect();
			return;
		}

		logger.debug("Sending new Blob in Valid E Ticket Message");
		ConnectionManagerInterfaceCoordinator.getConnection().sendData(
				this.getContext(), validMessage);

	}

	private void checkInWithTicket(ETicket localETicket)
			throws PersistenceException {

		logger.info("Allow check in for: " + this.customerBlob.getUserName());
		logger.debug("Creating new trip entry for User ID: "
				+ this.customerBlob.getUserId());
		...
		DataControllerInterfaceCoordinator.getPersistentDataController().save(
				saveList);
		logger.debug("Saved trip successfully");
	}

	/* private void checkInWithTicket(ETicket ticket) throws
	 * PersistenceException {
	 * 
	 * //Create local representation of ticket and check in with that //Note
	 * that here only a copy of the committed ETicket is made and stored. I.e. A
	 * ticket bought on this bus and //used again on this bus is also stored as
	 * a copy //transform e ticket List<ETicket> sharedETicket = new
	 * ArrayList<ETicket>(); sharedETicket.add(ticket);
	 * 
	 * this.checkInWithTicket(convertETicketsSharedToLocal(sharedETicket).get(0))
	 * ; } */

	private List<ETicket> convertETicketsSharedToLocal(
			List<ETicket> sharedETicketList) {

		// Transform ETickets
		List<ETicket> localETicketList = new ArrayList<ETicket>();
		for (ETicket sharedETicket : sharedETicketList) {
			localETicketList.add(sharedETicket.clone());
		}
	...
	private void disconnect() {
		...
		try {
			...
			this.active = false;
			CustomerServiceController.getInstance().disconnect(
					this.getContext());
			ConnectionManagerInterfaceCoordinator.getConnection().sendData(
					this.getContext(), null);
		} catch (SendDataException e) {
			logger.error("Error while sending data to the communication controller, going to abort communication");
			// Aborting happens automatically since error while sending implies
			// connection problems => connection automatically terminates
			return;
		}
	}
