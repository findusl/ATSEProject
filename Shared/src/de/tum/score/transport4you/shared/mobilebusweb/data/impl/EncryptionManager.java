...
public class EncryptionManager {
	...
	public EncryptionManager() throws Exception {
		...
		// cipher still needs to be initialized before use according to encryption/decryption!
		this.cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
		
		// derive a secret key from the password using PBKDF2, this.password and an arbitrary salt
		SecretKeyFactory factory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1");
		KeySpec spec = new PBEKeySpec(this.password.toCharArray(), "T4YSALTED".getBytes(), 1000, 128);
	...
	public byte[] decryptSequence(byte[] sequence) throws Exception {
		...
		// obtain the ciphertext from the sequence (everything after the first 16 bytes)
