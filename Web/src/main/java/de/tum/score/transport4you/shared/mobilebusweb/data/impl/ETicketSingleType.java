...
public class ETicketSingleType extends ETicketType {
	...
	/**
	 * Single Tickets may come in batches of multiple tickets, this is the
	 * number of tickets in one batch.
	 */
	private int amountTickets;

	/**
	 * Since buying multiple tickets is cheaper, this defines a factor to
	 * calculate a discount on the total price.
	 * 
	 * Should be 0 <= <code>discountFactor</code> <= 1. Lower value means lower
	 * end price.
	 */
	...
	/**
	 * Calculates the total price of a ticket packet using the number of tickets
	 * contained and a discount factor;
	 * 
	...
	public double getTotalPrice() {
		...
		int amount = this.getAmountTickets();
		double discountFactor = this.getDiscountFactor();
		return price * amount * discountFactor;
	}
	...
	public String toString() {
		return "ETicketSingleType[" + "validMinutes=" + this.getValidMinutes() + "," + "amount="
				+ this.getAmountTickets() + "," + "discountFactor=" + this.getDiscountFactor() + "]";
	}
}
